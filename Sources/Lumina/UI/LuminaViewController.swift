...
open class LuminaViewController: UIViewController {
    ...
    var previewLayer: AVCaptureVideoPreviewLayer {
        ...
        guard let camera = camera, let layer = camera.getPreviewLayer() else {
    ...
    public func setZoom(factor: Float, animated: Bool = true) {
        ...
        guard let device = camera?.videoInput?.device else { return }
        do {
    ...
    open var isVideoStabilizationEnabled: Bool = false {
        didSet {
            ...
            camera?.isVideoStabilizationEnabled = isVideoStabilizationEnabled
        }
    }
    ...
    @objc func cameraDeviceDidChange(_ notification: Notification) {
        ...
        if device.position == .back && device.isVirtualDevice {
            if let factor = device.virtualDeviceSwitchOverVideoZoomFactors.first {
                wideAngleZoomFactor = Float(factor.doubleValue)
            } else {
    ...
    override open func viewDidLoad() {
        ...
        focusOverlayView = UIView(frame: view.bounds)
        ...
        view.addSubview(focusOverlayView)

        // Create the focus view, respecting the custom focusImage if provided.
        let image = focusImage ?? UIImage(systemName: "camera.metering.partial")?.withTintColor(.white, renderingMode: .alwaysOriginal)
        ...
        focusOverlayView.addSubview(focusView)
    }
